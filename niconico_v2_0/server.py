...
DELTA = -240
INTERVAL = int(1 / 60 * 6000)
SIZE_BASE = 30
IMAGE_SIZE_BASE = 256
DIFFUSE_DELTA = 120
DIFFUSE_TRANSPARENT_DELTA = -10
DIFFUSE_TRANSPARENT_PLUS_TIME = 0.5
...
HTML_BASE = open(dirPath+"/index.html", "r", -1, "utf8").read()
...
class Motions(tkinter.Toplevel):
    ...
    def _diffusion(self):
        ...
        self._W += dx * 2
        self._H += dy * 2
        self._a += self.dif_a * (t - self._t) * DIFFUSE_TRANSPARENT_PLUS_TIME
...
class ImageWindow(Motions):
    def _deploy(self,
        ...
        self._content = tkinter.Canvas(self,
                                       width=int(w * rate),
                                       height=int(h * rate),
                                    #    fill=TRANSPARENT,      this is error (unknown option "-fill")
                                       )
...
def main_server(host="", port=2525):
    ...
    threading.Thread(target=server.serve_forever, daemon=False).start()     # if thread not start(), cannot access localhost html file
    print("server ready")
    return server
